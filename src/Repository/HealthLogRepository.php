...
class HealthLogRepository extends ServiceEntityRepository
{
    ...
    public function getDateRangeBounds(
        ...
        if (null === $result || null === $result['minTs']) {
    ...
    public function findAggregatedByDateRange(
        ...
        // `default` is load-bearing, not defensive padding: $interval arrives as
        // a plain string from the controller, and without it an unexpected value
        // would throw \UnhandledMatchError from deep inside a read endpoint.
        $fmt = match ($interval) {
            'day' => '%Y-%m-%d',
            'week' => '%Y-W%W',
            'month' => '%Y-%m',
